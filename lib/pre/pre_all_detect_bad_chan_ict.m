function json_data = pre_all_detect_bad_chan_ict(json_data,mat_path,varargin)
...
for i=1:n_segm
    ...
    % load
    load([mat_path '/' json_eeg_fn{i}],'eeg_data');
    
    pre_dur = json_data(i).eeg_pre;
    pre_data = eeg_data(:,1:((pre_dur-10)*eeg_fs)); % Only detect noise in the preictal segment (excluding 10 seconds before seizure)
    
    % detect bad channels and add to json structure
    json_data(i).bad_chan_detected = ...
        pre_detect_bad_chan(pre_data,eeg_channels,json_data(i).eeg_fs,varargin{:});
