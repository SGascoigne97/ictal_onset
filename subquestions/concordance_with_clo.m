...
for det = det_meths
    if det == "clo"
        ...
        concord_tab.(sprintf(det)) = table(final_output.Patient_id, nan(size(final_output,1), 1),...
        ...
        for subj = 1:size(final_output)
            ...
            onset_mat = final_output(subj,:).(sprintf("%s_%s", det, chan_or_roi)){:};
            ...
            % Skip subjects with no consensus onset or CLO
             ...
             concord_tab.(sprintf(det)).max_clo_concord(subj) = max(per_sz_concord);
             ...
             concord_tab.(sprintf(det)).consensus(subj) = cohensKappa(logical(clo), onset_cons);
        end
        
        % Compute AUC and p-values comparing outcome groups
        auc_tabs.(sprintf(chan_or_roi)).clo_concord.(sprintf(det)) = outcome_auc(concord_tab.(sprintf(det)), ["consensus", "max_clo_concord"],...
            concord_tab.(sprintf(det)).outcome == out_grps(2), "n_perm",n_perm,"plot_fig",1,"save_fig",save_fig,...
            "save_loc",save_loc, "file_type", "svg", "main_title", sprintf("AUCs comparing outcome groups based on %s concordance with CLO %s", det, chan_or_roi));
%   
        %% VISUALISATIONS 
        % Half violins plot distributions of markers for each outcome group 
        % (ILAE 1-2 compared against ILAE 3+)
        half_violin_upd(concord_tab.(sprintf(det)),  ["consensus", "max_clo_concord"],...
            concord_tab.(sprintf(det)).outcome==out_grps(2), "grp_names", out_grps,...
            "y_lim", [-0.55, 1.05;-0.55, 1.05],"save_fig", save_fig, "file_type", "svg",...
            "save_loc", sprintf("%shalf_violins_%s",save_loc, det),...
        ...
        for concord_summ = ["consensus", "max_clo_concord"]
            nexttile
            non_nan = ~isnan(concord_tab.(sprintf(det)).(sprintf(concord_summ)));
            concord_var = categorical(concord_tab.(sprintf(det)).(sprintf(concord_summ))>0.4,[0,1],["Below 0.4", "Above 0.4"]);
            outcome_var = categorical(concord_tab.(sprintf(det)).outcome == out_grps(2),[0,1],out_grps);
            ...
            colorbar off
            title(sprintf("%s %s (X^2 =%.2f , p = %.3f)", det, concord_summ, chi2,p))
        end
        saveas(f, sprintf("%s%s_contingency_moderate_concord.%s", save_loc, det, file_type))
    end
end
