...
%             onset_plhg(plhg_id,sz) = 1;
        ...
        % CLO and resection are based off of all channels (not just those
        % left after preprocessing)
        [soz_roi, soz_roi_str] = chan_to_roi_crit(channel_details.is_soz, incl_roi, unq_roi, 'threshold', reg_thresh);
        [resected_roi, resected_roi_str] = chan_to_roi_crit(channel_details.is_resected5, incl_roi, unq_roi, 'threshold', reg_thresh);
