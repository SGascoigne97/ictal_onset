%% Onset detection for imprint detection potential paper 
clear all
close all
...
for pat = 14:length(patients)
    patient = patients{pat};
    fprintf("Patient %s (%d/%d) \n", patient, pat, length(patients))
%     if any(ismember(final_output.Patient_id, patient))
%         continue
%     end
    ...
    if all(cellfun(@isempty, json_data(1).channel_details.ROIname) == 1)
        fprintf("Patient %s does not have ROI information \n", patient)
        continue
    end
    [ch2roi_map_mat,ch_names,roi_names, ch_det_used]=map_to_roi(json_data,pat_data,atl);

    for sz = 1:size(pat_data, 1)
        rm_chan = ~ismember(strrep(pat_data(1,:).segment_channel_labels{:},' ',''),...
            strrep(ch_names,' ', ''));
        pat_data(sz,:).segment_data{:}(rm_chan,:) = [];
        pat_data(sz,:).segment_channel_labels{:}(rm_chan,:) = [];
    end
...
%%
    if size(pat_data,1) > 0
        ...
        [pat_data, pat_meta, cell_imprint,  sz_count_pat] = ...
            calc_imprint(pat_data, pat_meta, 'window_overlap', 7/8,...
    ...
    onset_output = [{patient}, {pat_data.segment_id}, {ch_names}, ...
        {roi_names}, {ch2roi_map_mat}, auto_det_onset,...
...
save(sprintf('tables/all_pat_with_focal_%d', atl),"final_output")
