...
%         opts.hsc       (1,1) double {mustBeNumeric, mustBePositive} = 0.1 %percentage of seizure to be ignored at the start for calculation of max spread
        opts.fig_ind (1,1) double {mustBeNumeric}= -1 % -1: no plotting, 0:new figure, k>0: figure with index k
...
%     hsc=opts.hsc;
    ...
    preict_buffer=10; % TO DO: add this as an optional argument
...
%     cell_t = cell(nsegs,1);
    ...
    for s = 1:nsegs
        ...
        if szd>6
            ...
            tsBaseline = tsdata(:,1:((sztbl.segment_pre-preict_buffer)*Fs))';
